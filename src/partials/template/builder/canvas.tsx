...
import { item, text } from "../../../store/types";
...
function Canvas() {
	...
	const setActiveItem = (item: item) => {
		dispatch(actions.templates.setActiveItem(item));
	};
	...
	useEffect(() => {
		async function downloadURI() {
			if (stageRef) {
				...
				let pathref = `${store.user.organization}/templates/${store.templates.currentTemplate._id}/example/${name}`;
				...
				>
					<Layer>
						...
						{store.templates.grid.isEnabled && (
							<Grid
								props={store.templates.grid}
								canvas={store.templates.currentTemplate.canvas}
							/>
						)}
					</Layer>
				</Stage>
